...
export default function IndividualStays({ heroConfig = {} }) {
  ...
  const handleSelectHotel = (hotel) => {
    ...
    setShowDropdown(false);
    loadHotels({ location: hotel.name });
  };

  const buildParams = ({ month = selectedMonth, price = null, search = searchInput } = {}) => {
    const params = {};
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="w-full bg-[#FFF8F2] rounded-xl shadow-sm border border-[#FFF0E0] px-6 sm:px-10 py-6 sm:py-8 mb-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-end gap-4 md:gap-0 md:divide-x md:divide-[#E0D4C8]">
            ...
            >
              ...
              {showBudgetSlider && priceBounds && (
                <div className="absolute top-full left-0 mt-2 bg-white rounded-xl shadow-lg border border-[#E0D4C8] px-6 pt-6 pb-5 z-50 w-72">
                  <p
                    className="text-xs tracking-[0.14em] uppercase text-[#181818] mb-4"
                    style={{ fontFamily: "Lato, sans-serif" }}
                  >
                    Price per night
                  </p>
          ...
          <div className="border-t border-[#E0D4C8] pt-4">
            <AdvancedFilters onApply={handleAdvancedFiltersApply} className="mb-0" disabled={!hasSearched} />
          </div>
        </div>
